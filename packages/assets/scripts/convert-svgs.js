  ...
  files.forEach(file => {
    ...
    innerContent = innerContent
      ...
      .replace(/style="([^"]*)"/g, (match, styleStr) => {
        ...
        if (styleStr.includes('fill-opacity:0') || styleStr.includes('fill:none')) {
          return 'fill="none"';
        }

        // Replace target colors with currentColor
        let newStyle = styleStr;
        const featureColors = [/#61f3f7/gi, /#5ffbf9/gi, /#44ffff/gi, /#00f0ff/gi, /#b6aef1/gi]; // Added #b6aef1 as some assets use it for features
        ...
        const styleObj = newStyle.split(';').reduce((acc, rule) => {
          ...
          if (key && val) {
            const reactKey = key.trim().replace(/-./g, x => x[1].toUpperCase());
      ...
      .replace(/(fill|stroke)="([^"]+)"/g, (match, attr, colorVal) => {
        ...
        const featureHexes = ['#61f3f7', '#5ffbf9', '#44ffff', '#00f0ff', '#b6aef1'];
...
${exports.map(e => `import { ${e.component} } from './${e.name}';`).join('\n')}
...
`;
  ...
  // Generate index.ts
  fs.writeFileSync(path.join(outputDir, 'index.ts'), `export * from './all';\n${exports.map(e => `export * from './${e.name}';`).join('\n')}`);
}
