...
const ACBotShape = ({
  ...
  const innerShadowId = useMemo(() => `inner-shadow-${Math.random().toString(36).substr(2, 9)}`, [])
  ...
  const getFill = (color?: string | [number, number, number]) => {
    ...
    if (Array.isArray(color)) return `hsl(${color[0]}, ${color[1]}%, ${color[2]}%)`
    return color
  }
  ...
  const renderBackground = () => {
    ...
    if (backgroundType === 'gradientLinear' && backgroundColors && backgroundColors.length >= 2) {
      const gradientId = `grad-${Math.random().toString(36).substr(2, 9)}`
      return (
        <>
          <defs>
            <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%" gradientTransform={`rotate(${backgroundRotation})`}>
              {backgroundColors.map((color, i) => (
                <stop key={i} offset={`${(i / (backgroundColors.length - 1)) * 100}%`} stopColor={getFill(color)} />
              ))}
            </linearGradient>
          </defs>
          <rect x="-50%" y="-50%" width="200%" height="200%" fill={`url(#${gradientId})`} />
        </>
      )
    }
    ...
    if (backgroundType === 'glass') {
      ...
      const colors = backgroundColors && backgroundColors.length >= 2 ? backgroundColors : ['#6366f1', '#a855f7']
      ...
      return (
        <g id="glass-background">
          ...
          <g filter={`url(#${filterId})`} opacity="0.6">
            ...
            {colors[2] && <circle cx="150" cy="300" r="100" fill={getFill(colors[2])} />}
          </g>
          <rect x="-50%" y="-50%" width="200%" height="200%" fill="rgba(255,255,255,0.02)" />
        </g>
      )
    }
    ...
    // Default: solid
    return (
      <rect x="-50%" y="-50%" width="200%" height="200%" fill={getFill(backgroundColors?.[0])} />
    )
  }

  const EmojiFilter = () => (
    <filter id={innerShadowId} x="0" y="0" width="128" height="128" filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
        ...
        <g transform="translate(-5, 34) scale(0.0929)">
          <g>
            ...
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${s}%, ${Math.max(0, l - 15)}%)` : "#7ca2ce"}
            />
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${s}%, ${Math.min(100, l + 20)}%)` : "#dbdfea"}
            />
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${s}%, ${Math.max(0, l - 25)}%)` : "#3b61a1"}
            />
            <path
            ...
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${Math.max(0, s - 30)}%, ${Math.min(100, l + 10)}%)` : "#b1b7cd"}
            />
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${s}%, ${Math.max(0, l - 30)}%)` : "#406198"}
            />
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${s}%, ${Math.max(0, l - 40)}%)` : "#27387d"}
            />
            <path
            ...
            <path
              ...
              fill={isCustom ? `hsl(${h}, ${Math.max(0, s - 20)}%, ${Math.min(100, l + 20)}%)` : "#b3b8ce"}
            />
            <path
