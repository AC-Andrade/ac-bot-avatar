...
if (!fs.existsSync(outputDirEyes))
  fs.mkdirSync(outputDirEyes, { recursive: true });
if (!fs.existsSync(outputDirMouths))
  fs.mkdirSync(outputDirMouths, { recursive: true });
...
function extractSection(sectionName, outputDir, suffix) {
  const startRegex = new RegExp(
    `export const ${sectionName}: Record<${
      sectionName === "eyes" ? "EyeType" : "MouthType"
    }, React\\.ReactNode> = {`
  );
  ...
  // Generate index.ts
  let indexContent = exportedKeys
    .map((key) => `import { ${toPascalCase(key)}${suffix} } from './${key}';`)
    .join("\n");
  indexContent += `\n\nexport * from './all';\n`;
  indexContent +=
    `\nexport {\n` +
  ...
  let allContent = `import type { ${
    sectionName === "eyes" ? "EyeType" : "MouthType"
  } } from '@acandrade/ac-bot-avatar-core';\n`;
  allContent += `import React from 'react';\n`;
  allContent += exportedKeys
    .map((key) => `import { ${toPascalCase(key)}${suffix} } from './${key}';`)
    .join("\n");
  allContent += `\n\nexport const ${sectionName}Map: Record<${
    sectionName === "eyes" ? "EyeType" : "MouthType"
  }, React.FC> = {\n`;
  allContent += exportedKeys
    .map((key) => `  ${key}: ${toPascalCase(key)}${suffix},`)
    .join("\n");
  allContent += `\n};\n`;
